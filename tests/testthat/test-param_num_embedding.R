test_that("num_embedding() returns a quantitative param object", {
  p <- num_embedding()
...
test_that("num_embedding() has the correct default range and label", {
  p <- num_embedding()

  expect_equal(dials::range_get(p, original = TRUE), list(lower = 8L, upper = 64L))
  expect_equal(p$label, c(num_embedding = "# Embedding Dimensions"))
  expect_equal(p$inclusive, c(lower = TRUE, upper = TRUE))
  expect_null(p$trans)
})

test_that("num_embedding() accepts a custom range", {
  p <- num_embedding(range = c(16L, 32L))

  expect_equal(dials::range_get(p, original = TRUE), list(lower = 16L, upper = 32L))
})

test_that("num_embedding() works with grid and value utilities", {
  p <- num_embedding()
  ...
  expect_named(grid, "num_embedding")
})
